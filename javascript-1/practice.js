...
forTheLoveOfGeorge = notGeorge.map(function change(el){
	return el ="George"
	console.log(el)
})
...
people.filter(function murder(el, i, arr){
	if(el.friend === false){
		enemies.push(el)
	}
	return enemies
...
let totallyAwesome = people.reduce(function total(total, num){
	return total + Math.round(num.awesomeLevel)
}, 0)
